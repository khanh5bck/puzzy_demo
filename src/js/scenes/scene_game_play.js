...
export class SceneGamePlay extends Scene_Base {
  constructor(setting) {
    super(setting)
    GameManager.game_play_state = GAME_PLAY_STATE.WAITING
    this.state
    this.grid = new Array()
    ...
    this.timer = GameManager.timerManager.createTimer(1000);

    GameManager.current_game_play = this
    this.message_start = this.messageStartGame()
    
    // sound.add('sound_move',CONFIG.url_sound_move);
    // sound.add('sound_win',CONFIG.url_sound_win);
    // sound.add('sound_game_over',CONFIG.url_sound_game_over);
  ...
  setupScene() {
    ...
    const space_y = 0;
    let banner_texture = PIXI.Texture.from('bl')
  ...
  startGame() {
    ...
    GameManager.game_play_state = GAME_PLAY_STATE.PLAYING
  ...
  settingCountDownMode(){
    let countdown = CONFIG.countdown_time*(GameManager.current_game_level+1);
  ...
  checkGameComplete() {
    if (this.checkTilesPosition()) {
      GameManager.game_play_state = GAME_PLAY_STATE.FINISH;
      this.timer.stop()
  ...
  messageStartGame() {
    if (GameManager.game_play_state==GAME_PLAY_STATE.WAITING) {
      const style = new PIXI.TextStyle({
        fontFamily: 'Arial',
        fontSize: 46,
        fontStyle: 'italic',
  ...
  swapTile(obj1, obj2) {
    ...
    const tween = GameManager.tweenManager.createTween(obj1);
    tween.from({ x: obj1.x, y: obj1.y }).to({ x: obj2.x, y: obj2.y })
    tween.time = this.move_speed;
    tween.on('end', () => {
      GameManager.keylock_state = false;
  ...
  logicMoveTile(vector) {
    ...
    if (GameManager.keylock_state
      || GameManager.game_play_state != GAME_PLAY_STATE.PLAYING) {
      return
    }
    GameManager.keylock_state = true
  ...
  destroyScene() {
    this.timer.stop()
    GameManager.timerManager.removeTimer(this.timer)
